...
public class ConnectionContext {
	...
	private void finalize(InternalMessage message) {
		InternalMessage.create()//
				...
				.build()//
				.post();
        bus.post(WebRTCEvents.MEDIA_LOCAL_STREAM_CREATED.occurFor(slave.getSession()));
        bus.post(WebRTCEvents.MEDIA_STREAMING.occurFor(master.getSession()));
        bus.post(WebRTCEvents.MEDIA_STREAMING.occurFor(slave.getSession()));
	}


	private void answerRequest(InternalMessage message) {
        bus.post(WebRTCEvents.MEDIA_LOCAL_STREAM_CREATED.occurFor(master.getSession()));
		InternalMessage.create()//
                .from(master)//
                .to(slave)//
				...
				.build()//
				.post();
        bus.post(WebRTCEvents.MEDIA_LOCAL_STREAM_REQUESTED.occurFor(slave.getSession()));
	}
	...
	public void begin() {
        ...
        bus.post(WebRTCEvents.MEDIA_LOCAL_STREAM_REQUESTED.occurFor(master.getSession()));
	}
